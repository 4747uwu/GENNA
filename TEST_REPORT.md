...
| −1.5 | **`rss_bytes()` returned 0 on macOS for every measurement.** The probe had a Windows branch and an "everything else" branch that read `/proc/self/statm`; macOS has no `/proc`, so `fopen` returned NULL and the function returned 0. `oocore_test` then asserted `rss_m * 2 < rss_c` — i.e. `0 < 0` — and printed *"mapped open is at least 2x lighter (0.0 MB vs 0.0 MB)"*. Found when CI ran on macOS for the first time. | The out-of-core claim (**0.7 MB resident vs 32.6 MB**) had never been measured on a Mac at all. Fixed with a real `task_info`/`MACH_TASK_BASIC_INFO` implementation, plus a check that RSS is measurable *before* two readings are compared — 0 means "could not measure", never "measured zero". macOS now reports 33.4 MB vs 1.8 MB, 18.7×. |
